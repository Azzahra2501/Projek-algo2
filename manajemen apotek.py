...
# ==================== GENERATE BY SISTEM ====================
def cek_stok_dan_generate_rekomendasi_greedy(data):
    ...
    obat_unik = {d['Nama']: d for d in data}.values()
    obat_sort_by_stok = sorted(obat_unik, key=lambda x: int(x['Total Stok']))

    for obat in obat_sort_by_stok:
        if int(obat['Total Stok']) < 20:
            for d in data:
                if d['Nama'] == obat['Nama']:
                    d['Rekomendasi Restock'] = 'Ya'
        else:
            for d in data:
                if d['Nama'] == obat['Nama']:
                    d['Rekomendasi Restock'] = 'Tidak'
...
def generate_rekomendasi_stok():
    ...
    data_diperbarui = cek_stok_dan_generate_rekomendasi_greedy(data)

    data_diperbarui.sort(key=lambda x: int(x.get('Total Stok', 0)))

    simpan_data(data_diperbarui)

    print("\n=== Generate by Sistem: Cek Stok  ===")
...
def main_kadaluarsa():
    os.system("cls" if os.name == "nt" else "clear")
    print("=== Cek Obat Kadaluarsa (DFS Traversal) ===")
